...
async function createChatSubsystem(options = {}) {
    ...
    const provider = options.provider || new LocalOpenAIProvider({ endpoint: config.inferenceUrl, model: config.model, quantization: config.modelQuantization, contextTokens: config.contextTokens, structuredOutputMode: config.structuredOutputMode })
