...
async function execute(interaction) {
    ...
    const stream = interaction.options.getBoolean('stream') ?? true
    ...
    await dependencies.repository.collection(COLLECTIONS.migrations).updateOne(
        ...
        { $set: { schemaVersion: 1, migrationId: 'chat-runtime-config', mode: globalThis.operating_mode, model, stream, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } },
        { upsert: true }
    )
    await interaction.editReply(`Chat is ${globalThis.operating_mode}; model is ${model}. Cloud inference remains disabled.`)
}
