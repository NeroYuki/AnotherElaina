...
const server_address = process.env.BOT_ENV === 'lan' ? 'http://192.168.1.5:7050' : 'http://192.168.196.142:7050'
...
module.exports = {
    ...
    sendFinishBeatmap(msgRef, audio_file, audio_filename, beatmap_path, beatmap_info, user_id, image_file = null, image_filename = null, request_server_address = server_address) {
        ...
        return new Promise(async (resolve, reject) => {
            const resultMsgRef = await msgRef.channel.send({ content: `Beatmap generation finished, finalizing beatmap...` });
...
BeatmapSetID:-1`);
            ...
            zip.finalize().then(async () => {
                ...
                if (fileSizeInBytes > 9.9 * 1024 * 1024) {
                    ...
                    await resultMsgRef.channel.send({ content: `<@${user_id}> Beatmap finalization finished, beatmap size exceed 10MB, sending beatmap link...`, embeds: [embeded] });
                }
                else {
                    // send the zip file to the channel
                    await resultMsgRef.channel.send({ content: `<@${user_id}> Beatmap finalization finished, sending beatmap...`, files: [`./temp/${zip_filename}`] });
                }
                ...
                console.log(err);
                resultMsgRef.edit({ content: "Failed to finalize/upload beatmap file" });
                reject(err);
                return
            });
        })
    ...
    async switchToLocalServer(params, msgRef) {
    ...
    createActionButtons(interaction, includeSwitch = true) {
    ...
    async handleActionButtons(interaction, params, client, msgRef, onTimeout = null) {
        const filter = i => {
            i.deferUpdate();
            return i.user.id === interaction.user.id;
        };

        return msgRef.channel.awaitMessageComponent({ filter, time: 300000 })
            .then(async i => {
                if (i.customId.startsWith('switchtocpu_')) {
                    const success = await this.switchToLocalServer(params, msgRef);
                    if (success) {
                    ...
                    this.cancelMapperTask(params, client, msgRef);
                    return;
                }
            })
            .catch(err => {
                ...
                if (onTimeout) {
                    ...
                    msgRef.edit({ content: `<@${params.user_id}> Request timed out, please try again later` });
                    this.cancelMapperTask(params, client, msgRef);
                }
            });
    },
    ...
    async execute_inference(interaction, params, client, msgRef = null) {
        ...
        const is_using_gpu = ['http://192.168.1.5:7050','http://192.168.196.142:7050'].includes(params.server_address)
        ...
        if (!health || (health && is_using_gpu && !is_gpu_having_enough_vram)) {
            ...
            await this.handleActionButtons(interaction, params, client, msgRef, () => {
        ...
        try {
            ...
            if (err.response && err.response.status === 409 && is_non_local_server && params.model === 'v30') {
                ...
                const row = this.createActionButtons(interaction, true);
                ...
                const message_content = { content: `<@${params.user_id}> The server is currently busy with another generation. You can switch to CPU generation (slower) or cancel.`, components: [row]}
