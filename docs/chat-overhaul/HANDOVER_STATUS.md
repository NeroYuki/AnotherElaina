...
- Qwen 3.8 structured output exposed an upstream llama.cpp failure: server-side JSON grammar can raise `Unexpected empty grammar stack after accepting piece: / (14)` and leave that loaded process generating repeated `/` tokens even for ordinary prompts. `CHAT_STRUCTURED_OUTPUT_MODE=auto` now avoids server grammar for Qwen from the first request, supplies the trusted schema in the prompt, strips only a whole-response JSON code fence, and retains application schema/semantic validation. Other models keep server grammar, with a one-time prompt fallback for the same upstream exception. Provider/transport failures are no longer mislabeled as extraction-repair validation failures, and repeated-token output fails explicitly as `INFERENCE_DEGENERATE_OUTPUT`. A corrupted Qwen process must be unloaded once after the old bot worker is stopped or upgraded.
